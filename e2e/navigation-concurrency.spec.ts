...
test("repeated clicks cannot bypass a pending confirmation", async ({ page }) => {
...
test("repeated Back keeps the first destination and matching rendered content", async ({ page }) => {
  await page.goto("/page1");
  await page.getByRole("link", { name: "Page2", exact: true }).click();
...
test("browser history cannot replace a pending link destination", async ({ page }) => {
  await page.goto("/page1");
  await page.getByRole("link", { name: "Guard playground" }).click();
...
test("unmounting a pending guard cancels it and releases navigation", async ({ page }) => {
...
test("the first destination owns confirmation until it is settled", async ({ page }) => {
  await page.goto("/page1");
  await page.getByRole("checkbox", { name: "Enable Navigation Guard" }).check();
  await page.getByRole("checkbox", { name: "Use Async Confirm" }).check();
