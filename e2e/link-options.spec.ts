...
test("accepted replacement links preserve browser history", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "router.back()", exact: true }).click();
  await expect(page).toHaveURL("/page1");
  await expect(page.getByText("Current Page: 1")).toBeVisible();
});

test("guarded link callbacks run once and cancellation remains available", async ({ page }) => {
